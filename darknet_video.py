...
def parser():
    ...
    parser.add_argument("--output_prefix", type=str, default=None,
                        help="prefix of the files resulting from inference")
...
def inference(darknet_image_queue, detections_queue, fps_queue):
    ...
    while cap.isOpened():
        ...
        if args.output_prefix is not None and len(detections) != 0:
            new_file = open(args.output_prefix + str(nth_frame) + ".txt", mode='w')
            for i in range(len(detections)):
...
def drawing(frame_queue, detections_queue, fps_queue):
    ...
    video = set_saved_video(cap, args.out_filename, (original_width, original_height))
    nth_frame = 0
    ...
    while cap.isOpened():
        ...
        if frame is not None:
            ...
            if args.out_filename is not None:
                video.write(image)
            if args.output_prefix is not None and len(detections) != 0:
                cv2.imwrite(args.output_prefix + str(nth_frame) + "_inference.jpg", image)
                cv2.imwrite(args.output_prefix + str(nth_frame) + ".jpg", frame)
            nth_frame += 1
